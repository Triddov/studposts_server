...
def new_post():
    ...
    try:
        # Извлекаем информацию о пользователе из JWT токена
        identity = get_jwt_identity()
        user_login = identity['login']
        user_password = identity['password']  # сохраняем также пароль для проверки
        ...
        if image_data:
            ...
            if not is_image_valid(image_data) or not is_icon_square(image_data):
                response.set_status(420)
                return response.send()

        # Проверка на запрещённые слова в заголовке и содержании
        title = BannedWordsChecker.bad_words(title)
        content = BannedWordsChecker.bad_words(content)
        ...
        unique_post_id = generate_uuid()

        # Создание нового поста в базе данных
        try:
            post_id = Post.create_post(unique_post_id, user_login, title, content, tags, image_data)
            response.set_data({'post_id': post_id, 'unique_post_id': unique_post_id})
            return response.send()
