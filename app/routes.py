...
def edit_userprofile():
    ...
    try:
        user_id = get_jwt_identity()  # Получаем идентификатор пользователя из JWT токена
        ...
        is_valid, validation_error = check_user_data(data, updating=True)
        if not is_valid:
            response.set_status(417)
            response.set_message(validation_error)
            return response.send()
        ...
        # Валидация и сохранение новой фотографии профиля
        if pers_photo_data:
            header, pers_photo_data = pers_photo_data.split(",", 1)
        ...
        try:
            User.update_user(user_id, first_name, middle_name, sur_name, password, email, phone_number, pers_photo_data)
